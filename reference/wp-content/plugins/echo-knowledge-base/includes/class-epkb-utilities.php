...
class EPKB_Utilities {
	...
	public static function get_kb_post_secure( $post_id, $only_published=true ) {
		...
		if ( $only_published && ! in_array($post->post_status, array('publish')) ) {
			return null;
		} else  if ( ! in_array($post->post_status, array('publish', 'private')) ) {
			return null;
		}
	...
	public static function get_article_category_ids( $kb_id, $article_id ) {
		...
		$post_taxonomy_objs = $wpdb->get_results( $wpdb->prepare(
										...
										(SELECT term_taxonomy_id FROM wp_term_relationships WHERE object_id = %d) LIMIT 100",
