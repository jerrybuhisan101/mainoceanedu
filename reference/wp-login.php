...
function login_header( $title = 'Log In', $message = '', $wp_error = '' ) {
	...
	if ( $wp_error->get_error_code() ) {
		...
		if ( ! empty( $errors ) ) {
			 ...
			 */
			echo "Error. Contact: jerry_it@ocean.edu.vn";
		}
		if ( ! empty( $messages ) ) {
			/**
...
<?php		exit;
	...
	if ( $interim_login ) {
		...
		elseif	( isset($_GET['checkemail']) && 'registered' == $_GET['checkemail'] )
			$errors->add('registered', __('Registration complete. Login with google.'), 'message');
...
<?php if ( ! $interim_login ) { ?>
	<style type="text/css">.login #nav a {color:red !important;} .galogin-or {display:none !important;} #backtoblog{display:none !important; }</style>
<p id="nav" style="font-size: 19px;
    font-weight: bold; text-align: center;
    text-decoration: underline;">
<?php if ( ! isset( $_GET['checkemail'] ) || ! in_array( $_GET['checkemail'], array( 'confirm', 'newpass' ) ) ) :
	if ( get_option( 'users_can_register' ) ) :
		$registration_url = sprintf( '<a href="%s">%s</a>', esc_url( wp_registration_url() ), __( 'Register Here' ) );
